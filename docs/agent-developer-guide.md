...
Open the app in your dev store, go to the dashboard, and click "Run" next to your agent in the sidebar.
...
- **`"done"`** -- "Handled Overnight" section. Agent fixed something automatically.
- **`"action_needed"`** -- "Needs Your Decision" section. Merchant must approve/decide.
- **`"insight"`** -- "Insights" section. Informational, no action required.
...
const response = await admin.graphql(`
  query {
    products(first: 10) {
      edges {
        node {
          id title description vendor productType
          variants(first: 5) {
            edges {
...
Add `@anthropic-ai/sdk` to dependencies if using Claude:
```bash
